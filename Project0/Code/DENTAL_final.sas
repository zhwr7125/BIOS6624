...
/*read in the data*/
proc import DATAfile="D:\PRO0\data_processed\Project0_dental_data.csv" out=dental_data
	DBMS=CSV REPLACE;
	GETNAMES=YES;
/*	     DATAROW=2; 
	guessingrowS=32767;*/
...
proc freq data=dental;
	where attach_ind=0;
	table race_2*race / missprint;
	table sex_2*sex / missprint;
run;
proc freq data=dental;
	where attach_ind=0;
...
run;
*Welch's ANOVA for d_attach 0.0303;
...
*0.2160;
quit;
*Sex violated the assumption for ANOVA;
/*D_ATTACH*/
...
*After check, his data is not very bad, but let's remove him and fit again;
ods graphics on;
proc glm data=dental PLOTS=(DIAGNOSTICS RESIDUALS);
	where id^=104;
	class trtgroup(ref='1') sex(ref='0') race_2(ref='0') smoker(ref='0');
